...
void nSigma_Electron_Cut_efficiency()
{
  ...
  for(int ipt=0;ipt<NpT_bins_run12_MB;ipt++) {
    ...
    TH1F *heff;
    TH1F *nsigmaE_MB=new TH1F("nsigmaE_MB","",NpT_bins_run12_MB,pt_run12_MB);
	...
	// // Gaus->SetParLimits(2,heff->GetRMS()-100*heff->GetRMSError(),heff->GetRMS()+100*heff->GetRMSError());
	 ...
	 nsigmaE_MB->SetBinContent(ipt+1,Gaus->GetParameter(1));
	 nsigmaE_MB->SetBinError(ipt+1,Gaus->GetParameter(2));
    ...
    nsigmaE_MB->SetMarkerStyle(20);
    nsigmaE_MB->SetMarkerSize(1);
    nsigmaE_MB->SetMarkerColor(2);
