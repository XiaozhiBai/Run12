...
void Fit_electron_nsigma_Mean(TH1F *mh1NisgmaE_unlike[NpT_bins_run12_MB],TH1F *mh1NisgmaE_like[NpT_bins_run12_MB],TH1F *mh1NisgmaE_unlike_like[NpT_bins_run12_MB])
{
  ...
  TH2F *h5_a=new TH2F("h5_a","",10,1,4,100,-1,1.5);
  h5_a->GetXaxis()->SetTitle("p_{T} (GeV/c)");
  h5_a->GetYaxis()->SetTitle("Electron nsigma Mean");
    
  
  TF1 *fmean=new TF1(TString("fmean"),"[0]",1,4);
  TF1 *fmean_u=new TF1(TString("fmean_u"),"[0]",1,4);
  TF1 *fmean_d=new TF1(TString("fmean_d"),"[0]",1,4);
  TF1 *fsigma=new TF1(TString("fsigma"),"[0]",1,4);
  TF1 *fsigma_u=new TF1(TString("fsigma_u"),"[0]",1,4);
  TF1 *fsigma_d=new TF1(TString("fsigma_d"),"[0]",1,4);  
  ...
  Mean->Fit(fmean,"R","same",1,4); 
  Mean_u->Fit(fmean_u,"R0","same",1,4); 
  Mean_d->Fit(fmean_d,"R0","same",1,4); 
  
  Sigma->Fit(fsigma,"R","same",1,4);
  Sigma_u->Fit(fsigma_u,"R0","same",1,4);
  Sigma_d->Fit(fsigma_d,"R0","same",1,4);
  ...
  TH2F *h6=new TH2F("h6","",10,1,4,100,-7,1);
  ...
  TH1F * nsigma_e_kaon=(TH1F *) nsigma_E_kaon->Rebin(NpT_bins_run12_MB,"nsigma_E_kaon",pt_run12_MB); 
   ...
   TH2F *h7=new TH2F("h7","",10,1,4,100,-8,1);
