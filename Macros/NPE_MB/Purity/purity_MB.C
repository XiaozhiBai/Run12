...
//  gStyle->SetOptStat(00000);
  ...
  //   nsigmaE_inclusive_Trig2->Draw("COLZ");
  ...
  for(Int_t ipt=0;ipt<NpT_bins_run12_MB;ipt++)
    {
      ...
      //    TF1 *   total_4 =new TF1(TString("total_4"),"[0]*TMath::Gaus(x,[1],[2],1) + [3]*TMath::Gaus(x,[4],[5],1) + [6]*TMath::Gaus(x,[7],[8],1)+[9]*TMath::Gaus(x,[10],[11],1)",-15,15);
      TF1 *   total_4 =new TF1(TString("total_4"),"[0]*TMath::Gaus(x,[1],[2],1) + [3]*TMath::Gaus(x,[4],[5],1) + [6]*TMath::Gaus(x,[7],[8],1)",-15,15);
      ...
      //      total_4->SetParNames("k C","k #mu","k #sigma","p C","p #mu","p #sigma","#pi C","#pi #mu","#pi #sigma","e C","e #mu","e #sigma");

      total_3->SetLineColor(kBlue);
      total_4->SetLineColor(kBlue);
      ...
      if(ipt<6)
	{
      ...
      Float_t k_p_mean=0.5*(kaon_Mean->GetBinContent(ipt+1)+proton_Mean->GetBinContent(ipt+1));
      ...
      //-----------------------------------------------------------------------------------------------
      total_4->SetParameter(1,mh1_Kaon_mean->GetBinContent(ipt+1));
      total_4->SetParameter(2,mh1_Kaon_sigma->GetBinContent(ipt+1));

      total_4->SetParLimits(1,mh1_Kaon_mean->GetBinContent(ipt+1)-2,mh1_Kaon_mean->GetBinContent(ipt+1)+2);
      total_4->SetParLimits(2,mh1_Kaon_sigma->GetBinContent(ipt+1)-0.1,mh1_Kaon_sigma->GetBinContent(ipt+1)+0.1);

      total_4->SetParameter(4,mh1_Pion_mean->GetBinContent(ipt+1));
      total_4->SetParameter(5,mh1_Pion_sigma->GetBinContent(ipt+1));

      total_4->SetParLimits(4,mh1_Pion_mean->GetBinContent(ipt+1)-0.5,mh1_Pion_mean->GetBinContent(ipt+1)+0.5);
      total_4->SetParLimits(5,mh1_Pion_sigma->GetBinContent(ipt+1)-0.1,mh1_Pion_sigma->GetBinContent(ipt+1)+0.1);

      total_4->SetParameter(7,mh1_Proton_mean->GetBinContent(ipt+1));
      total_4->SetParameter(8,mh1_Proton_sigma->GetBinContent(ipt+1));

      total_4->SetParLimits(7,mh1_Proton_mean->GetBinContent(ipt+1)-0.5,mh1_Proton_mean->GetBinContent(ipt+1)+0.5);
      total_4->SetParLimits(8,mh1_Proton_sigma->GetBinContent(ipt+1)-0.1,mh1_Proton_sigma->GetBinContent(ipt+1)+0.1);

      total_4->SetParameter(10,electron_Mean->GetBinContent(ipt+1));
      total_4->SetParameter(11,electron_Sigma->GetBinContent(ipt+1));
      
      total_4->SetParLimits(10,electron_Mean->GetBinContent(ipt+1)-3*electron_Mean->GetBinError(ipt+1),electron_Mean->GetBinContent(ipt+1)+3*electron_Mean->GetBinError(ipt+1));
      total_4->SetParLimits(11,electron_Sigma->GetBinContent(ipt+1)-3*electron_Sigma->GetBinError(ipt+1),electron_Sigma->GetBinContent(ipt+1)+3*electron_Sigma->GetBinError(ipt+1));
      //     total_3->SetParLimits(9,100,10000);
      // total_4->SetParLimits(10,electron_Mean->GetBinContent(ipt+1)-1,electron_Mean->GetBinContent(ipt+1)+1);
      // total_4->SetParLimits(11,electron_Sigma->GetBinContent(ipt+1)-10*electron_Sigma->GetBinError(ipt+1),electron_Sigma->GetBinContent(ipt+1)+10*electron_Sigma->GetBinError(ipt+1));



      //nsigmaE_inclusive[ipt]->Fit(total_3,"R","same",-10,4);
           nsigmaE_inclusive[ipt]->Fit(total_4,"R","same",-14,4);
      ...
      g1->SetParameter(0,total_4->GetParameter(6));
      g1->SetParameter(1,total_4->GetParameter(7));
      g1->SetParameter(2,total_4->GetParameter(8));
      
      g2->SetParameter(0,total_4->GetParameter(3));
      g2->SetParameter(1,total_4->GetParameter(4));
      g2->SetParameter(2,total_4->GetParameter(5));
      
      g3->SetParameter(0,total_4->GetParameter(0));
      g3->SetParameter(1,total_4->GetParameter(1));
      g3->SetParameter(2,total_4->GetParameter(2));

      g4->SetParameter(0,total_4->GetParameter(9));
      g4->SetParameter(1,total_4->GetParameter(10));
      g4->SetParameter(2,total_4->GetParameter(11));
	...
	mPurity_sys=abs(purity_mean-mPurity_temp_threeSigma);
      ...
      Fit_purity(nsigmaE_inclusive[ipt],ipt,electron_Mean->GetBinError(ipt+1),electron_Sigma->GetBinError(ipt+1),total_3,g1,g2,g3,3,1); 
       ...
       TFile *file_purity=new TFile("purity_MB.root","RECREATE");
       purity_MB->Write();
       file_purity->Close();
