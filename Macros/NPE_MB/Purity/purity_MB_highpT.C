...
char buf[1024];
  ...
  Double_t bin_low=purity_mean-200*purity_err;
  Double_t bin_high=purity_mean+200*purity_err;
  ...
  if(ipt==29)
    {
      bin_low=purity_mean-5000*purity_err;
      bin_high=purity_mean+5000*purity_err;
    }
  
  if(ipt==30)
      {
      bin_low=purity_mean-500*purity_err;
      bin_high=purity_mean+500*purity_err;
    }
...
void Draw_purity(TH1D * purity)
{
  ...
  c2->cd();
  TH2F *h2=new TH2F("h2","",10,1,4,10,0,1.1);
  h2->Draw();
